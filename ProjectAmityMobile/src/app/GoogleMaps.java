...
public class GoogleMaps {
    ...
    public Image retrieveStaticImage(int width, int height, double lat, double lng, int zoom,
       ...
       // byte[] imageData = loadHttpFile("http://maps.google.com/maps/api/staticmap?center=1.34538,103.934312&format=jpg&zoom=15&size=400x400&markers=color:blue|label:A|1.34538,103.934312&sensor=false");

        return Image.createImage(imageData, 0, imageData.length);
    }
    public Image retrieveStaticImage2(int width, int height, double lat, double lng, int zoom,
            String format) throws IOException {
        byte[] imageData = loadHttpFile(getMapUrl(width, height, lng, lat, zoom, format));
       // byte[] imageData = loadHttpFile("http://maps.google.com/maps/api/staticmap?center=1.34538,103.934312&format=jpg&zoom=15&size=400x400&markers=color:blue|label:A|1.34538,103.934312&sensor=false");
    ...
    private String getMapUrl(int width, int height, double lng, double lat, int zoom, String format) {
        return "http://maps.google.com/maps/api/staticmap?center=" + lat + "," + lng +"&format="
                + format + "&zoom=" + zoom + "&size=" + width + "x" + height + "&markers=color:blue|"+ lat + "," + lng+ "&sensor=false"; //"&key=" + apiKey;
    //http://maps.google.com/maps/api/staticmap?center=1.34538,103.934312&format=jpg&zoom=15&size=400x400&markers=color:blue|label:A|1.34538,103.934312&sensor=false
    }
        private String getMapUrl2(int width, int height, double lng, double lat, int zoom, String format) {
        return "http://maps.google.com/maps/api/staticmap?center=" + lat + "," + lng +"&format="
                + format + "&zoom=" + zoom + "&size=" + width + "x" + height + "&markers=color:blue|"+ lat + "," + lng+ "&sensor=false"; //"&key=" + apiKey;
    //http://maps.google.com/maps/api/staticmap?center=1.34538,103.934312&format=jpg&zoom=15&size=400x400&markers=color:blue|label:A|1.34538,103.934312&sensor=false
    }
